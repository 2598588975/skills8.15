...
1. 检查源素材：读取时长、分辨率、帧率和音轨；视频可用时运行：
   `python scripts/extract_video_evidence.py <video-path> --out-dir <output-dir> --frames 12`
2. 按视频实际动作节点划分时间段。短视频优先使用 1–3 秒区间；用户要求逐秒时，使用 1 秒区间并覆盖完整时长。
3. 只写可见、可听或由素材明确提供的事实。看不清时标记“画面证据不足”；听不清时写“无可辨识台词”，不得虚构。
4. 从空间关系、机位、肢体动作和物理反馈解释画面，不用“高级、好看、震撼、电影级”等空洞词。
5. 保持主体身份、服装、道具位置、动作方向、视线和轴线连续。
6. 用户要求故事板图片时，优先使用视频真实截图排版；每格下方标注准确的“起始秒–结束秒”。不要用 AI 重绘替代证据帧，除非用户明确要求手绘故事板。
7. 最后给出一段可直接用于目标视频模型的中文提示词，并说明复刻成功率、主要风险和输入建议。
